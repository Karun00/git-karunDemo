  insert into [dbo].[MaterialCodeMaster] (GroupCode,MaterialCode,MovementType,ServiceType,UOM,Remarks,RecordStatus,CreatedBy,
  CreatedDate,ModifiedBy,ModifiedDate,MaterialDescription,IsCalculated,Chargedas,PortCode,QuayCode,BerthCode,ChargedFor)
  values('900003',126,'ARMV',null,'HR','Workboat Craft Assistance','A',1,getdate(),1,getdate(),'Workboat Craft Assistance','Y',1
  ,'MB','SPM','SPM','WBCA')

  insert into [dbo].[MaterialCodeMaster] (GroupCode,MaterialCode,MovementType,ServiceType,UOM,Remarks,RecordStatus,CreatedBy,
  CreatedDate,ModifiedBy,ModifiedDate,MaterialDescription,IsCalculated,Chargedas,PortCode,QuayCode,BerthCode,ChargedFor)
  values('900003',126,'SGMV',null,'HR','Workboat Craft Assistance','A',1,getdate(),1,getdate(),'Workboat Craft Assistance','Y',1
  ,'MB','SPM','SPM','WBCA')
...
insert into [dbo].[MaterialCodePort] (MaterialCodeMasterid,PortCode)   values
((select MaterialCodeMasterid  from [MaterialCodeMaster] 
  where MaterialCode=126 and MovementType='ARMV' and PortCode='MB' and BerthCode='SPM'),'MB')
insert into [dbo].[MaterialCodePort] (MaterialCodeMasterid,PortCode)   values
((select MaterialCodeMasterid from [MaterialCodeMaster] 
  where MaterialCode=126 and MovementType='SGMV' and PortCode='MB' and BerthCode='SPM'),'MB')
insert into [dbo].[MaterialCodePort] (MaterialCodeMasterid,PortCode)   values
((select MaterialCodeMasterid from [MaterialCodeMaster] 
  where MaterialCode=126 and MovementType='ARMV' and PortCode='MB' and BerthCode='CBM'),'MB')
insert into [dbo].[MaterialCodePort] (MaterialCodeMasterid,PortCode)   values
((select MaterialCodeMasterid from [MaterialCodeMaster] 
  where MaterialCode=126 and MovementType='SGMV' and PortCode='MB' and BerthCode='CBM'),'MB')
...
--                       and MP.PortCode = @portcode
  ...
  -- CHARGED FOR COMPRESSED AIR
      ...
      union 
    -- BERTH DUES
     SELECT  ISPOSTED,ResourceAllocationID,VCN,MovementName,ServiceName,GroupCode,MaterialCode,
                  MaterialDescription , AccountNo, StartTime, Endtime,IsCalculated,
                  Chargedas,UOM , MovementType, ServiceType,
                  ServiceReferenceType, OperationType, TaskStatus, RecentlyPostedDate,
                  PostingDateTime,Chargedas AS DueHours, Chargedas AS TotalHours, CloseMterReding, 
                  startmtrreding,MeterSerialNo, BerthName
         FROM (
              select isnull(RPD.RevenuePostingDtlID,'0') ISPOSTED, VesselCallID ResourceAllocationID,V.VCN,
              'Arrival' as MovementName,upper(M.MaterialDescription) as ServiceName,
              M.GroupCode GroupCode,M.MaterialCode MaterialCode, upper(M.MaterialDescription) MaterialDescription ,
                  RP.SAPAccNo AS AccountNo,
              ATB  as StartTime,ATUB  as Endtime,'N' as IsCalculated,
              convert(varchar,(ceiling(DATEDIFF(mi,V.ATB , V.ATUB )/1440.0))-@cntBnk) as Chargedas,M.UOM ,
                (ceiling(DATEDIFF(mi,V.ATB , V.ATUB )/1440.0)-@cntBnk) as ChargedasFORwhere,
                     '' as MovementType,'' as ServiceType,
                     '' as ServiceReferenceType,'' as OperationType, '' as TaskStatus,
                     @postedon RecentlyPostedDate,
                   getdate() PostingDateTime,
                 -- '1' AS 'DueHours',  '1' AS 'TotalHours',
                         '0' CloseMterReding, 
                                 '0' as startmtrreding,
                             ''  MeterSerialNo, '' BerthName
      ...
      left join RevenuePostingDtl RPD on  RPD.VCN = an.VCN
                                          and rpd.ReferenceID =  V.VesselCallID
                                          and rpd.GroupCode =M.GroupCode
                                          and rpd.MaterialCode = M.MaterialCode 
   
           left join RevenuePosting RP ON RP.RevenuePostingID = RPD.RevenuePostingID  
              where V.VCN  = @BERTDUEVCN 
              and V.ATUB is not null
             )bt WHERE  ChargedasFORwhere > 0
...
BEGIN
    ...
    --  REFUSE REMOVAL
                select RPD.RevenuePostingDtlID ISPOSTED ,  VC.VesselCallid ResourceAllocationID,
                 VC.VCN,'Arrival' MovementName , UPPER(M.MaterialDescription) ServiceName,
                   M.GroupCode, M.MaterialCode  , UPPER(M.MaterialDescription) MaterialDescription ,
                   RP.SAPAccNo AS AccountNo,
                           VC.BreakWaterIn StartTime, VC.BreakWaterOut Endtime,

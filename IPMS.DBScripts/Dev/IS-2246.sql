Update MaterialCodeMaster set RecordStatus = 'I'  where MaterialCode = 110
...
set @cntCrewflag = ( select count(VM.ServiceRequestID) from VesselCallMovement VM
left join MaterialCodeMaster BR on vm.FromPositionPortCode = br.PortCode 
						and vm.FromPositionQuayCode = br.QuayCode
				        and vm.FromPositionBerthCode = br.BerthCode
						   and BR.ChargedFor='ILND' 
where vcn  = @VCN and FromPositionBerthCode is not null
    ...
    */
      ...
      */
      ...
      -- CREW TRANS PORTATION
                select distinct isnull(RPD.RevenuePostingDtlID,'0') ISPOSTED , RA.ServiceReferenceID ,
                   VC.VCN,
				    CASE
                            WHEN (@cntCrewflag	> 0)
                               THEN  S.SubCatName 
                     ...
                     CASE
						...
						CASE
                            WHEN (@cntCrewflag	> 0)
                               THEN  convert(varchar, (datediff(day,VC.ATB,VC.ATUB)+1)) 
                            else
                                 convert(varchar, (datediff(day,V.BreakWaterIn ,V.BreakWaterOut)+1)) 
                            END AS Chargedas,
				 ...
				 inner join VesselCall V on v.vcn=VC.VCN
                  ...
                  AND (VC.MovementType ='ARMV' or 
				    VC.MovementType =
				 CASE WHEN @cntCrewflag > 0 THEN
				 'SHMV'
				   ELSE
				   'ARMV'
			   END
      ...
      select 
        ...
        inner join ArrivalNotification an on an.VCN = V.VCN 
          INNER JOIN MaterialCodePort MP on an.PortCode = MP.PortCode
       INNER JOIN MaterialCodeMaster M ON M.MaterialCodeMasterid = MP.MaterialCodeMasterid
                                        and   M.ChargedFor ='SMSA' and M.RecordStatus = 'A'

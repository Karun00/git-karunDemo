...
namespace IPMS.Notifications.Engine
{
    ...
    {
       private FuelRequisitionVO _fuelRequisition;
       private IFuelRequisitionRepository _fuelRequisitionRepository;

       public FuelRequisitionNotifier(Notification pendingNotification, NotificationTemplateVO notificationTemplate, Entity entityDetails, List<NotificationRole> roles)
        ...
        public override List<User> GetUsersToBeNotified()
        {
            ...
            if (notificationTemplate.NotificationTemplateBase == "U")
                _userslist.AddRange(_userRepository.GetUserDetailsForRoleAndPortCodeByUserType(GetPortCode(), "", "", _fuelRequisition.CreatedBy));
            else
            {

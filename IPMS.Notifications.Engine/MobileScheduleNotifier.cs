...
{
    public class MobileScheduleNotifier : Notifier
    {
        private ResourceAllocationVO _resourceAllocation;
        private IMobileScheduledTasksRepository _mobileScheduledTasksRepository;

        public MobileScheduleNotifier(Notification pendingNotification, NotificationTemplateVO notificationTemplate, Entity entityDetails, List<NotificationRole> roles)
            : base(pendingNotification, notificationTemplate, entityDetails, roles)
        {
            try
            {
                XmlConfigurator.Configure();
                log = LogManager.GetLogger(typeof(MobileScheduleNotifier));

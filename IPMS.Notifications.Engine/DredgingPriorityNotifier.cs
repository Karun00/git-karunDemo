...
{
    public class DredgingPriorityNotifier : Notifier
    {
        private DredgingPriority _dredgingPriority;
        private IDredgingPriorityRepository _dredgingPriorityRepositoy;

        public DredgingPriorityNotifier(Notification pendingNotification, NotificationTemplateVO notificationTemplate, Entity entityDetails, List<NotificationRole> roles)
            : base(pendingNotification, notificationTemplate, entityDetails, roles)
        {
            //try
            //{
                XmlConfigurator.Configure();
                log = LogManager.GetLogger(typeof(DredgingPriorityNotifier));
                _dredgingPriorityRepositoy = new DredgingPriorityRepository(_NotifierunitOfWork);
                if (pendingNotification != null)
                {
                    _dredgingPriority = _dredgingPriorityRepositoy.GetDredgingPriorityDetailsById(pendingNotification.Reference);
        ...
        public override List<User> GetUsersToBeNotified()
        {
            ...
            {
                _userslist.AddRange(_userRepository.GetUserDetailsForRoleAndPortCodeByUserType(GetPortCode(), "", "", _dredgingPriority.CreatedBy));
            }
        ...
        {
            return Common.GetTokensDictionary(entityDetails, _dredgingPriority);
        }
    }
}

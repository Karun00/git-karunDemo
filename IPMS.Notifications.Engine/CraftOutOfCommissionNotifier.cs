...
{
    public class CraftOutOfCommissionNotifier : Notifier
    {
        private CraftOutOfCommissionVO _craftOutOfCommission;
        private ICraftOutOfCommissionRepository _craftOutOfCommissionRepository;

        public CraftOutOfCommissionNotifier(Notification pendingNotification, NotificationTemplateVO notificationTemplate, Entity entityDetails, List<NotificationRole> roles)
            : base(pendingNotification, notificationTemplate, entityDetails, roles)
        {
            try
            {
                XmlConfigurator.Configure();
                log = LogManager.GetLogger(typeof(CraftOutOfCommissionNotifier));
                _craftOutOfCommissionRepository = new CraftOutOfCommissionRepository(_NotifierunitOfWork);
                _craftOutOfCommission = _craftOutOfCommissionRepository.GetCraftOutCommissionDetailsById(pendingNotification.Reference);
            }
            catch (Exception ex)
            {
        ...
        public override List<User> GetUsersToBeNotified()
        {
            ...
            {
                _userslist.AddRange(_userRepository.GetUserDetailsForRoleAndPortCodeByUserType(GetPortCode(), "", "", _craftOutOfCommission.CreatedBy));
            }
        ...
        {
            return Common.GetTokensDictionary(entityDetails, _craftOutOfCommission);
        }

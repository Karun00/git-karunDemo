...
{
    public class ExternalDivingOnCompletionNotificationNotifier : Notifier
    {
        public ExternalDivingRegisterVO _externalDivingRegisterVO;
        protected IExternalDivingRegisterRepository _externalDivingRepository;

        public ExternalDivingOnCompletionNotificationNotifier(Notification pendingNotification, NotificationTemplateVO notificationTemplate, Entity entityDetails, List<NotificationRole> roles)
            : base(pendingNotification, notificationTemplate, entityDetails, roles)
        {
            try
            {
                ...
                _externalDivingRepository = new ExternalDivingRegisterRepository(_NotifierunitOfWork);
                _externalDivingRegisterVO = _externalDivingRepository.GetExternalDivingDetailsOnCompletion(pendingNotification.Reference);
            }
            catch (Exception ex)
            {
        ...
        {
            return _externalDivingRegisterVO.PortCode;
        }
        ...
        public override List<User> GetUsersToBeNotified()
        {
            ...
            if (notificationTemplate.NotificationTemplateBase == "U")
                _userslist.AddRange(_userRepository.GetUserDetailsForRoleAndPortCodeByUserType(GetPortCode(), pendingNotification.UserType, "", _externalDivingRegisterVO.ModifiedBy));
            else
            {
        ...
        public override Dictionary<string, string> GetTokensDictionary()
        {
            ...
            return Common.GetTokensDictionary(entityDetails, _externalDivingRegisterVO);
        }
    }
}

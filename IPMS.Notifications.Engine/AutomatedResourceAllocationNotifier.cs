...
{
    public class AutomatedResourceAllocationNotifier : Notifier
    {
        private ResourceAllocationVO _resourceAllocation;
        private IAutomatedResourceSchedulingService _automatedResourceAllocationService;
        public AutomatedResourceAllocationNotifier(Notification pendingNotification, NotificationTemplateVO notificationTemplate, Entity entityDetails, List<NotificationRole> roles)
            : base(pendingNotification, notificationTemplate, entityDetails, roles)
        {
            try
            {
                log = LogManager.GetLogger(typeof(AutomatedResourceAllocationNotifier));
                _automatedResourceAllocationService = new AutomatedResourceSchedulingService(_NotifierunitOfWork);
                _resourceAllocation = _automatedResourceAllocationService.GetResourceAllocationById(Convert.ToInt32(pendingNotification.Reference, CultureInfo.InvariantCulture));
            }
            catch (Exception ex)
            {

(function (IPMSRoot) {
    var AccountLoginViewModel = function () {

        var self = this;

        self.viewModelHelper = new IPMSROOT.viewModelHelper();
        self.validationHelper = new IPMSRoot.validationHelper();
        self.accountModel = ko.observable(new IPMSROOT.AccountLoginModel());
        self.forgotPasswordModel = ko.observable(new IPMSRoot.ForgotPasswordModel());
        ...
        ResetUserPwd = function (model) {
            ...
            if (errors == 0) {
                self.viewModelHelper.apiPut('api/User/PutForgetPassword', ko.mapping.toJSON(model),
                      function Message(data) {
                          toastr.options.closeButton = true;
                          toastr.options.positionClass = "toast-top-right";
                          toastr.success("Login Credentials Updated Successfully", "Forget Password");
        ...
        checkLogindet = function (model) {
            ...
            var errors = self.CPValidation().errors().length;
            if (errors == 0) {
                //self.viewModelHelper.apiPost('api/Account/CheckCredentials', ko.mapping.toJSON(model),
                //               function Message(data) {
                //                   if (isMobile.any() != null) {
                //                       if (isMobile.any().length >= 1) {
                ...
                //                   }
                //                   else {
                //                       var isChecked = $('#chkTermsConditions').prop('checked');
                //                       if (isChecked) {
                //                           if (data.IsFirstTimeLogin == "Y") {
                //                               window.location.href = "/ChangePassword/" + data.IsFirstTimeLogin;
                //                           }
                //                           else {
                //                               if (model.ReturnUrl() != '' && model.ReturnUrl().length > 1) {
                //                                   window.location.href = "/" + model.ReturnUrl().substring(1);
                ...
                //                           toastr.options.closeButton = true;
                //                           toastr.options.positionClass = "toast-top-right";
                //                           toastr.warning("Please ensure you have accepted terms and conditions", "Login");
    ...
    IPMSRoot.AccountLoginViewModel = AccountLoginViewModel;
}(window.IPMSROOT));
...
namespace Core.Repository.Providers.EntityFramework
{
    internal class StoredProcedureParser
    {
        ...
        public static string GetStoreProcedureName(object storedProcedure)
        {
            ...
            if (attribute == null)
                throw new InvalidOperationException(String.Format(
                    "{0} is not decorated with StoredProcedureAttribute.", storedProcedure.GetType(),CultureInfo.InvariantCulture));
        ...
        private static string BuildSql(object storedProcedure, IEnumerable<StoredProcedureParameterInfo> parameterInfos)
        {
            ...
            var execSql = String.Format("EXEC {0} ", storedProcedureName,CultureInfo.InvariantCulture);

...
namespace Core.Repository
{
    public class Repository<TEntity> : IRepositoryAsync<TEntity> where TEntity : EntityBase
    {
        ...
        public virtual IQueryable<TEntity> SelectQuery(string query, params object[] parameters) { return _dbSet.SqlQuery(query, parameters).AsQueryable().AsNoTracking(); }
        ...
        public IQueryFluent<TEntity> Query() { return new QueryFluent<TEntity>(this).Tracking(false); }

        public virtual IQueryFluent<TEntity> Query(IQueryObject<TEntity> queryObject) { return new QueryFluent<TEntity>(this, queryObject).Tracking(false); }

        public virtual IQueryFluent<TEntity> Query(Expression<Func<TEntity, bool>> query) { return new QueryFluent<TEntity>(this, query).Tracking(false); }
...
Expression<Func<TEntity, bool>> query = null,
            ...
            int? page = null,
            int? pageSize = null,
            bool tracking = false) { return Select(query, orderBy, includes, page, pageSize, tracking).AsEnumerable(); }
    }
}